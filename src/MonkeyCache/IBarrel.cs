...
namespace MonkeyCache
{
	public interface IBarrel
	{
		...
		void AddObject<T>(string key, T data, TimeSpan expireIn, string eTag = null, JsonSerializerSettings jsonSerializationSettings = null);
		void Empty(params string[] key);
		void EmptyAll();
		void EmptyExpired();
		bool Exists(string key);
		string Get(string key);
		T GetObject<T>(string key, JsonSerializerSettings jsonSettings = null);
